...
export function updateGroupByVrid(vrId = null) {
    return (dispatch, getState) => {
        ...
        if (selectedVrId) {
            Modals.Group.findByVrid(selectedVrId).then((list) => {
...
export function updateGroupArr(list) {
    return (dispatch) => {
        Modals.Group.updateAllGroup(list).then(() => {
...
export function addGroup(title, vrId, groupId) {
    return (dispatch) => {
        Modals.Group.add({ title: title, id: groupId, vrId: vrId }).then(() => {
            dispatch(updateGroupByVrid());
        });
    };
}
...
export function deleteGroup(obj) {
    return (dispatch, getState) => {
        Modals.Group.delete(obj.id).then(() => {
            dispatch(updateGroupByVrid());
        });
    };
}
...
export function updateAllGroupMusic(arr, music1, music2) {
    ...
    return (dispatch) => {
        Modals.Group.updateAllGroup(objArr).then(() => {
            dispatch(updateGroupByVrid());
        });
    };
}
...
export function updateGroupMusic(obj, music1, music2) {
    return (dispatch) => {
        Modals.Group.update({ ...obj, music1, music2 }).then(() => {
            dispatch(updateGroupByVrid());
        });
    };
}
