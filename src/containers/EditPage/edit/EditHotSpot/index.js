...
import EditViewImage from "./EditViewImage";
...
class EditHotSpot extends Component {
			...
			return (
				<div style={{ borderBottom: "1px solid #eee" }}>
					<div>
						...
						<FlatButton color='primary' onClick={this.onAddHotspotClick.bind(this)}>
							添加热点
						</FlatButton>
					</div>
	...
	renderEditHotspot() {
		...
		const menuList = ["切换", "相册", "文本", "图文", "链接", "音频", "视频", "序列图"];
		if (hotpotSelected != null || isAdd) {
			return (
				<div>
					<div style={{ borderBottom: "1px solid #eee" }}>
						<span>
							...
							<FlatButton color='primary' onClick={this.handleCloseEditHotspot.bind(this)}>
								关闭
							</FlatButton>
						</span>
					</div>
					...
					<div>
						<SelectField value={this.state.hotSpotType} onChange={this.handleTypeChange.bind(this)} className={styles["select-container"]}>
						...
						<div className={styles["edit-type-container"]}>{this.renderEditByType()}</div>
					</div>
					<div style={{ position: "fixed", bottom: 0 }}>
						<FlatButton color='primary' variant='contained' onClick={this.onEditConfirmClick.bind(this)}>
							{"确定"}
						</FlatButton>
						{isAdd ? null : (
							<FlatButton color='secondary' style={{ marginLeft: 20 }} variant='contained' onClick={this.onEditDeleteClick.bind(this)}>
								{"删除"}
							</FlatButton>
						)}
					</div>
	...
	renderEditByType() {
		...
		const { hotpotSelected, sceneSelected, groupList } = this.props;
		let action = hotpotSelected ? hotpotSelected.action : "";
		let hotSpotTypeActions = {
			type1: () => {
				const { sceneList, folderId, vrId, allSceneList } = this.props;
				return <EditSelectScene action={action} ref={(ref) => (this.editEle = ref)} selectId={null} sceneList={sceneList} allSceneList={allSceneList} groupList={groupList} folderId={folderId} sceneSelected={sceneSelected} vrId={vrId}></EditSelectScene>;
			},
			type2: () => {
			...
			type8: () => {
				const { addPicture } = this.props;
				return <EditViewImage action={action} ref={(ref) => (this.editEle = ref)} addPicture={addPicture}></EditViewImage>;
			},
		};
