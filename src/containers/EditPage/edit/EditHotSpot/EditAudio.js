...
export default class EditAudio extends Component {
	...
	render() {
		...
		return (
			<div>
				...
				<TextField defaultValue={defaultTitle} id="with-placeholder" label="标题" placeholder="标题" margin="normal" inputRef={input => (this.titleRef = input)} />
				<br />
				<FlatButton color="primary" variant="contained" onClick={() => this.setState({ showListModal: true })}>
					{"从库中添加"}
				</FlatButton>

				<FlatButton color="primary" variant="contained" style={{ float: "right" }} onClick={() => this.setState({ showUploadModal: true })}>
					{"添加音乐"}
				</FlatButton>
	...
	renderMusic() {
		...
		if (url) {
			return <div style={style}>{showName}</div>;
		}
	}
