...
export default class EditViewImage extends Component {
    constructor() {
        super();
        this.zzs = React.createRef();
        this.zbfsc = React.createRef();
    ...
    getResult() {
        ...
        return JSON.stringify({ type: "viewImage", title: title, picArr: list, moreInfo, check, openInNewWindow });
    }
    ...
    render() {
        const { list, defaultTitle, defaultMoreInfo, defaultSummary, playType } = this.state;
        ...
        let picArr = list.map((item) => {
            ...
            return (
                <div style={{ display: "inline-block", margin: 5 }} key={item.pic.name}>
                    ...
                    <div style={titleStyle} title={item.pic.showName}>
                        {item.pic.showName}
                    </div>
                </div>
            );
        ...
        return (
            <div>
                ...
                <TextField id='with-placeholder' label='标题' placeholder='标题' margin='normal' defaultValue={defaultTitle} inputRef={(input) => (this.title = input)} /> */}

                <FlatButton style={{ display: "inline-block" }} variant='contained' color='primary' onClick={() => this.setState({ showPicListModal: true })}>
                    {"图片库添加"}
                </FlatButton>

                <FlatButton style={{ float: "right" }} variant='contained' color='primary' onClick={() => this.setState({ showUploadModal: true })}>
                    {"添加图片"}
                </FlatButton>

                <div style={{ width: "180px", margin: "0 auto", marginTop: 15 }}>{picArr}</div>

                <TextField id='with-placeholder' type='number' label='序列帧总帧数(帧)' placeholder='请输入序列帧总帧数(帧)' margin='normal' inputRef={(input) => (this.summaryRef = input)} />
                <div style={{ width: "180px", margin: "0 auto", marginTop: 15 }}>{picArr}</div>
                <TextField id='with-placeholder' label='填写文字介绍' placeholder='文字介绍' margin='normal' defaultValue={defaultMoreInfo} inputRef={(input) => (this.moreInfo = input)} />
                <TextField id='with-placeholder' label='填写更多内容' placeholder='更多内容' margin='normal' defaultValue={defaultMoreInfo} inputRef={(input) => (this.moreInfo = input)} />
                ...
                <div style={{ marginTop: 20 }}>交互方式</div>
                <Select onChange={this.handleSelectChange.bind(this)} value={playType}>
                    <MenuItem value='0'>图片自动轮播</MenuItem>
                    <MenuItem value='1'>点击图片播放下一张</MenuItem>
                </Select>
    ...
    onUploadConfirm(path, showName) {
        CopyImageTmpToImage(path).then(() => {
            ...
            setTimeout(() => {
                ...
                if (!sameImg) {
                    list = [{ pic: { name: name, showName: showName }, text: "" }];
                    this.setState({ list: list });
                }
            }, 300);
        });
    ...
    renderPicListModal() {
        if (this.state.showPicListModal) {
            return <PicListModal isSignel={true} onCancel={this.onLocalPicListCancel.bind(this)} onConfirm={this.onLocalPicListConfirm.bind(this)}></PicListModal>;
        }
    }
}
