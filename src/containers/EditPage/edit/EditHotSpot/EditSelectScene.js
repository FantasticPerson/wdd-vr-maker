...
export default class EditSelectScene extends Component {
	constructor() {
		super();
		this.state = { selectItem: null, type: 0, check: true };
	}
	...
	componentDidMount() {
		...
		if (action.length > 0) {
			...
			this.setState({ selectItem: obj.toItem, value: obj.type, check: obj.check, title: obj.title });
		}
	}

	getResult() {
		const { selectItem, type, check } = this.state;
		if (selectItem != null) {
			return JSON.stringify({ type: "switch", toItem: this.state.selectItem, switchType: type, check: check, title: selectItem.name });
		}
		return "";
	}
	...
	render() {
		const { selectItem } = this.state;
		const { sceneSelected, allSceneList, groupList } = this.props;
		let sceneItemStyle = {
			margin: "5px",
			height: "120px",
			width: "80px",
		...
		let sceneGroups = {};
		groupList.map((item) => {
			if (!sceneGroups[item.id]) {
				sceneGroups[item.id] = [];
				let ids = item.sceneListIds || [];
				ids.map((item2) => {
					if (item2 !== sceneSelected) {
                        let sItem = allSceneList.find((item3) => item3.id == item2);
                        if(sItem){
                            sceneGroups[item.id].push(sItem)
                        }
					}
				});
			}
