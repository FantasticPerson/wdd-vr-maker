...
class PicListModal extends Component {
	...
	render() {
		...
		return (
			<Dialog open onClose={this.onCancelClick.bind(this)} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
				<DialogTitle id="alert-dialog-title">{"选择图片"}</DialogTitle>
				<DialogContent style={{ width: "500px" }}>
					<div style={{ height: "300px", overflowY: "auto" }}>{list}</div>
				</DialogContent>
				<DialogActions>
					<FlatButton onClick={this.onCancelClick.bind(this)}>取消</FlatButton>,<FlatButton onClick={this.onConfirmClick.bind(this)}>确认</FlatButton>
				</DialogActions>
			</Dialog>
		);
	}
