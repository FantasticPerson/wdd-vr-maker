	...
	render() {
		...
		return (
			<Dialog open={true} onClose={this.onCancelClick.bind(this)} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
				...
				<DialogContent>
					<TextField id="with-placeholder" label="请输入场景名称" placeholder="请输入场景名称" margin="normal" inputRef={input => (this.titleRef = input)} defaultValue={title} />
				</DialogContent>
				<DialogActions>
					<Button onClick={this.onCancelClick.bind(this)}>取消</Button>,<Button onClick={this.onConfirmClick.bind(this)}>确认</Button>
				</DialogActions>
			</Dialog>
		);
	}
