...
export function getSelector(config) {
	return createSelector(
		...
		(title, showBack, vrList, hotpotList, sceneList, pathname, folderList, folderSelectedId, hotpotSelectId, sceneSelected, krpano, groupList, groupSelectId, showType, picList, audioList) => {
			...
			if (config.allSceneList) result.allSceneList = filterAllScene(sceneList, groupList, pathname);
...
function filterAllScene(list, groupList, pathname) {
	var vrId = pathname.split("/")[2];
	...
	for (let i = 0; i < groupList.length; i++) {
		let tIds = groupList[i].sceneListIds || [];
		ids = [...ids, ...tIds];
	}
	let tItems = items.filter((item) => ids.indexOf(item.id) >= 0);
	return tItems;
}
